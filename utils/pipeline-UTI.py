...
class Pipeline:
    ...
    def __init__(self, case_id: str):
        ...
        self.llm = ChatOpenAI(temperature=0.1, model="gpt-4-turbo")
...
"""),
    ...
    def _validar_firac_data(self, data: Dict[str, Any], origem: str = "FIRAC") -> None:
        ...
        for campo in campos_esperados:
            if campo not in data or not data[campo]:
                ...
                data[campo] = "[DADO NÃO DISPONÍVEL]"
        ...
        if firac_cache_path_json.exists() or firac_cache_path_raw.exists():
            try:
                ...
                if firac_cache_path_json.exists():
                    ...
                    return {'data': None, 'raw': raw, 'cached': True, 'summary_cached': summary_cached}
            except Exception as e:
                logger.warning(f"Falha ao ler FIRAC cache: {e}")

        facts_block = "\n".join([f"- {s}" for s in candidate_facts]) if candidate_facts else "(nenhum fato candidato extraído diretamente – use apenas o resumo)"
        prompt = (
            "Produza análise FIRAC estruturada em JSON válido UTF-8. Campos: facts (lista de strings), issue (string), rules (lista de strings), application (string), conclusion (string). "
            "NÃO invente fatos: limite-se às sentenças em 'FATOS_CANDIDATOS' abaixo; se precisar resumir, mantenha o núcleo sem adicionar elementos novos. "
            "Se 'FATOS_CANDIDATOS' estiver vazio, use somente o RESUMO. Se algo faltar, deixe vazio. Texto em português.\n\n"
            f"FATOS_CANDIDATOS:\n{facts_block}\n\nRESUMO:\n{summary}\n\nJSON:")
        try:
            resp = self.llm.invoke(prompt)
            ...
            import json
            data = json.loads(raw)
            self._validar_firac_data(data, origem="FIRAC")
            self._cache_firac(focus_key, data, raw)
            logger.info(f"[FIRAC] (LLM) Resultado: {json.dumps(data, ensure_ascii=False, indent=2)}")
            ...
            try:
                ...
                data_fb = {k: "[DADO NÃO DISPONÍVEL]" for k in ["facts", "issue", "rules", "application", "conclusion"]}
                import re
                for k in data_fb:
                    match = re.search(rf"{k}\s*[:\-]\s*(.+)", raw_fb, re.IGNORECASE)
                    if match:
                        data_fb[k] = match.group(1).strip()
                self._validar_firac_data(data_fb, origem="FIRAC-FALLBACK")
                self._cache_firac(focus_key, data_fb, raw_fb)
                logger.info(f"[FIRAC] (FALLBACK) Resultado: {json.dumps(data_fb, ensure_ascii=False, indent=2)}")
                return { 'data': data_fb, 'raw': raw_fb, 'cached': False, 'summary_cached': summary_cached, 'candidate_facts': candidate_facts }
            except Exception as e2:
                data_empty = {k: "[DADO NÃO DISPONÍVEL]" for k in ["facts", "issue", "rules", "application", "conclusion"]}
                logger.info(f"[FIRAC] (EMPTY) Resultado: {json.dumps(data_empty, ensure_ascii=False, indent=2)}")
                return { 'data': data_empty, 'raw': f'Erro ao gerar FIRAC: {e2}', 'cached': False, 'summary_cached': summary_cached, 'candidate_facts': candidate_facts }
    ...
    def analyze_firac(self, context: str) -> Dict[str, str]:
        return self.case_analyzer.analyze_firac(context)

    def generate_peticao_rascunho(self, dados_ui: Dict[str, Any], firac_results: Dict[str, str]) -> str:
        return self.petition_generator.generate_peticao_rascunho(dados_ui, firac_results)
