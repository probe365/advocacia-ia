...
class Pipeline:
    def __init__(self, case_id: str, tenant_id: Optional[str] = None):
        """
        Inicializa o Pipeline do caso jurídico, com isolamento completo por tenant.
        ...
        # ---------------------------------------------------------
        # 1) Identidades principais do pipeline
        # ---------------------------------------------------------
        self.case_id = str(case_id)
        self.tenant_id = tenant_id or "default"

        # ---------------------------------------------------------
        # 2) Diretórios multi-tenant
        # ---------------------------------------------------------
        self.base_dir = Path("./cases") / self.tenant_id / self.case_id
        ...
        for d in [self.uploads_dir, self.raw_dir, self.vectorstore_dir,
                  self.chunks_dir, self.cache_dir, self.exports_dir]:
            d.mkdir(parents=True, exist_ok=True)

        logger.info(
            f"[PIPELINE INIT] case_id={self.case_id}, tenant={self.tenant_id}, "
            f"case_dir={self.base_dir}"
        )
        ...
        try:
            ...
            logger.warning("Modelo 'pt_core_news_sm' não encontrado. Usando modelo vazio.")
        ...
        # ---------------------------------------------------------
        # 6) Cliente OpenAI nativo
        # ---------------------------------------------------------
        self.openai_client = OpenAI()

        # ---------------------------------------------------------
        # 7) Vector DB (Chroma local por caso)
        # ---------------------------------------------------------
        settings = Settings(
        ...
        # ---------------------------------------------------------
        # 9) Módulo de ingestão (PDF, imagens, áudio, vídeo, textos)
        # ---------------------------------------------------------
        
        self.ingestion_handler = IngestionHandler(
            case_dir=self.base_dir,
        ...
        # ---------------------------------------------------------
        logger.info(
            f"[PIPELINE READY] Estrutura criada em {self.base_dir} "
    ...
    def get_nlp():
        ...
        if _NLP_CACHE is None:
            _NLP_CACHE = spacy.load("pt_core_news_sm")
        return _NLP_CACHE



    # --- MÉTODO DE CHAT REFEITO 21Nov25 -----
    def _normalize_history(self, chat_history_raw: List[dict]):
        messages = []
        for h in chat_history_raw or []:
            role = h.get("role")
    ...
    def chat(self, user_query: str, chat_history: List[dict], search_scope: str = "case") -> Dict[str, Any]:
        ...
        """
        normalized_history = self._normalize_history(chat_history)
    ...
    def identify_legal_risks(self, focus: Optional[str] = None) -> str:
        ...
        try:
            resp = self.strong_llm.invoke(prompt)
    ...
    f"FATOS_CANDIDATOS:\n{facts_block}\n\nRESUMO:\n{summary}\n\nJSON:"
        ...
        try:
            ...
            try:
                resp2 = self.strong_llm.invoke(fallback_prompt)
    ...
    def delete_ementas_by_filename(self, filename: str) -> int:
        ...
        try:
            ...
            # 2. Deletar os documentos usando seus IDs
            logger.info(f"Encontrados {len(ids_to_delete)} chunks para deletar do arquivo '{filename}'.")
            self.ementas_kb_store.delete(ids=ids_to_delete)
            self.ementas_kb_store.persist()
            # Usando o método delete da coleção interna
