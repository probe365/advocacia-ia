...
# Classes de badge e ícones
TIPO_PARTE_BADGE_CLASSES = {
...
def _normalize_tipo_parte(tipo_parte: str | None) -> str:
    return (tipo_parte or "").strip().lower()

def get_tipo_parte_label(tipo_parte: str | None) -> str:
    """Retorna rótulo em português para tipo_parte."""
    normalized = _normalize_tipo_parte(tipo_parte)
    if not normalized:
        return '-'
    return TIPO_PARTE_LABELS.get(normalized, tipo_parte)
...
def get_tipo_parte_description(tipo_parte: str | None) -> str:
    ...
    if not normalized:
        return ''
    return TIPO_PARTE_DESCRIPTIONS.get(normalized, '')
...
def get_tipo_parte_badge_class(tipo_parte: str | None) -> str:
    ...
    return TIPO_PARTE_BADGE_CLASSES.get(normalized, 'badge-secondary')
...
def get_tipo_parte_icon(tipo_parte: str | None) -> str:
    ...
    return TIPO_PARTE_ICONS.get(normalized, 'fa-circle-question')
...
def validate_tipo_parte(tipo_parte: str | None) -> bool:
    ...
    return normalized in VALID_TIPOS_PARTE
...
def format_tipo_parte_for_display(tipo_parte: str | None) -> str:
    ...
    label = get_tipo_parte_label(normalized)
    badge_class = get_tipo_parte_badge_class(normalized)
    icon = get_tipo_parte_icon(normalized)
...
def get_tipos_parte_by_category(category: str) -> list:
    ...
    return list(TIPO_PARTE_BY_CATEGORY.get((category or "").lower(), []))
